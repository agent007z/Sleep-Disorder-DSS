...
# উন্নত কাস্টম CSS (বক্সের উচ্চতা ও ফন্ট সাইজ সামঞ্জস্য করা হয়েছে)
st.markdown("""
    <style>
    .main { background-color: #f8f9fa; }
    
    /* হেডিংগুলোর ফন্ট সাইজ */
    h3 { font-size: 32px !important; font-weight: 700 !important; color: #1a252f; margin-bottom: 20px !important; }
    h4 { font-size: 28px !important; font-weight: 600 !important; color: #2c3e50; margin-bottom: 15px !important; }
    
    /* ইনপুট লেবেলের ফন্ট সাইজ */
    label p {
    ...
    /* ড্রপডাউন এবং ইনপুট বক্সের উচ্চতা বৃদ্ধি (যাতে লেখা না কাটে) */
    ...
    /* ইনপুট ও ড্রপডাউনের ভেতরের টেক্সট সাইজ */
    ...
    /* স্লাইডারের টেক্সট সাইজ */
    div[data-baseweb="slider"] div {
        font-size: 22px !important;
    }
    
    /* ড্রপডাউন ওপেন হওয়ার পর অপশনগুলোর সাইজ ও স্পেসিং */
    ul[data-baseweb="menu"] li, ul[role="listbox"] li, li[role="option"] {
    ...
    .stButton>button {
        background: linear-gradient(90deg, #4CAF50 0%, #45a049 100%);
        color: white; font-weight: bold; border-radius: 12px;
        padding: 15px; font-size: 32px !important; border: none; transition: 0.3s;
    }
...
# --- নতুন হেডার বক্স (প্রেডিকশন বক্সের মতো ডিজাইন) ---
st.markdown("""
    <div class='prediction-box' style='background-color: #f1f8ff; border: 2px solid #b6d4fe; margin-top: 10px;'>
        <h1 style='color: #084298; margin: 0; font-size: 48px;'>Advanced Sleep Disorder Dashboard</h1>
        <p style='color: #444; font-size: 24px; margin-top: 10px;'>Provide your details below to analyze sleep health using AI.</p>
    </div>
""", unsafe_allow_html=True)
...
if predict_button and model:
    ...
    st.markdown(f"""
        <div class='prediction-box' style='background-color: {bg}; border: 2px solid {color}55;'>
            <h1 style='color: {color}; margin: 0; font-size: 48px;'>{status_text} {result_text}</h1>
            <p style='color: #666; font-size: 22px; margin-top: 10px;'>Predicted based on your clinical and lifestyle metrics.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    if is_healthy:
        st.markdown("""
        <div style='background-color: #e8f5e9; padding: 20px; border-radius: 8px; font-size: 26px; color: #2e7d32; font-weight: bold; margin-bottom: 20px; border-left: 6px solid #2e7d32;'>
             Great job! Your sleep profile looks healthy. Here is how you can maintain it:
        </div>
        <ul style='font-size: 24px; line-height: 1.8; color: #2c3e50;'>
            <li><b>Consistency:</b> Try to go to bed and wake up at the same time every day, even on weekends.</li>
            <li><b>Active Lifestyle:</b> Keep up with your daily physical activities and step count.</li>
            <li><b>Stress Management:</b> Continue managing your stress levels through hobbies or relaxation.</li>
        </ul>
        """, unsafe_allow_html=True)
        
    elif "Insomnia" in result_text:
        st.markdown("""
        <div style='background-color: #fff3cd; padding: 20px; border-radius: 8px; font-size: 26px; color: #856404; font-weight: bold; margin-bottom: 20px; border-left: 6px solid #ffc107;'>
        ...
        <ul style='font-size: 24px; line-height: 1.8; color: #2c3e50;'>
            ...
            <li><b>Limit Caffeine:</b> Avoid tea, coffee, or energy drinks in the late afternoon and evening.</li>
            <li><b>Relaxation Routine:</b> Try reading a book, deep breathing exercises, or a warm bath before sleeping.</li>
            <li><b>Environment:</b> Keep your bedroom dark, quiet, and comfortably cool.</li>
            <li><b>Medical Advice:</b> If sleep deprivation affects your daily life, please consult a doctor.</li>
        </ul>
        """, unsafe_allow_html=True)
        
    elif "Apnea" in result_text:
        st.markdown("""
        <div style='background-color: #f8d7da; padding: 20px; border-radius: 8px; font-size: 26px; color: #721c24; font-weight: bold; margin-bottom: 20px; border-left: 6px solid #dc3545;'>
        ...
        <ul style='font-size: 24px; line-height: 1.8; color: #2c3e50;'>
            ...
            <li><b>Sleep Position:</b> Try sleeping on your side instead of your back to keep your airways open.</li>
            <li><b>Weight Management:</b> If your BMI is in the overweight or obese category, reducing weight can significantly improve symptoms.</li>
            <li><b>Avoid Sedatives:</b> Avoid alcohol or sleeping pills before bed, as they can relax throat muscles and worsen breathing pauses.</li>
        </ul>
        """, unsafe_allow_html=True)
        
    else:
        st.markdown(f"""
        <div style='background-color: #d1ecf1; padding: 20px; border-radius: 8px; font-size: 26px; color: #0c5460; font-weight: bold; border-left: 6px solid #17a2b8;'>
